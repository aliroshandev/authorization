...
const Access = () => {
  ...
  } = useQuery(`/api/menus/client-id?clientId=${selectedClientId}`, getApi, {
  ...
  } = useQuery(`/api/resources/menu-id?menuId=${selectedMenuId}`, getApi, {
  ...
  } = useQuery("/api/roles?pageSize=100&currentPage=1", getApi);

  const { response: permissions } = useQuery(
    "/api/permissions?pageSize=100&currentPage=1",
    getApi
  );
